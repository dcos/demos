...
if [ "$(uname)" == "Darwin" ]; then
  ...
  sed -i '.tmp' "s,_ACCESS_KEY_ID,$ACCESS_KEY_ID,; s,_SECRET_ACCESS_KEY,$SECRET_ACCESS_KEY," ./drill/drill-s3-plugin-config.json
...
mv ./drill/drill-s3-plugin-config.json.tmp ./drill/drill-s3-plugin-config.json
