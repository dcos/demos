...
func init() {
	...
	flag.StringVar(&topic, "topic", "stream", "The kafka topic to be used. Example: topic1")
	flag.Usage = func() {
...
func main() {
	...
	for {
		...
		if source != target {
			rawmsg := fmt.Sprintf("%d %d %d %s", source, target, amount, string(city))
			msg := &sarama.ProducerMessage{Topic: topic, Value: sarama.StringEncoder(rawmsg)}
