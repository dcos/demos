...
func init() {
	...
	flag.StringVar(&topic, "topic", "transactions", "The name of the Kafka into which the data will be written.")
	flag.Usage = func() {
...
func main() {
	...
	for {
		...
		if source != target {
			...
			rawmsg := fmt.Sprintf("%s;%d;%d;%d", currentTime, source, target, amount)
